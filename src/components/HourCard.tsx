  ...
  return (
    <>
      {forecastDataInfo.forecast.forecastday[arrPosition].hour.map((hour: ForecastHourProps) => 
        ...
        <div id="hourCard" key={hour.time} className={`${hour.is_day === 0 ? `${hour.cloud > 30 ? 'cloudyNight' : 'night'} text-white` : `${hour.cloud > 30 ? 'cloudyDay' : 'day'} text-black`} flex flex-col items-center justify-center flex-shrink-0 w-full gap-1 text-xs border border-opacity-50 rounded-lg sm:text-sm sm:w-1/3 border-slate-500`}>
